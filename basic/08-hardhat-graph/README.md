...
- deploy the contract on sepolia and call trigger event.
...
4. TheGraph create a space of Subgraph
   ...
   Note: latest Graph CLI only support to deploy mainnet and sepolia. If you want to use it on the others networks, you need to login by Github account, then create and deploy it on Hosted Service.
...
graph-node:
  ...
  environment:
    ...
    # ethereum: 'dev:https://sepolia.infura.io/v3/INFURA_ID' # could connect to test networks
    RUST_LOG: info
```
